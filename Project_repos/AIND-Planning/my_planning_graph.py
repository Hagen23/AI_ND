...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # Connect action to current level if a_node prenodes are defined in S level
            if action_node.prenodes.issubset(s_level):
                self.a_levels[level].add(action_node)

                # Sets children - parent 
    ...
    def add_literal_level(self, level):
        ...
        # The literals to add are in the previous level of a_levels
        a_levels = self.a_levels[level - 1]

        for action_node in a_levels:
            for state_node in action_node.effnodes:
                # Add state_node to level
                self.s_levels[level].add(state_node)
                # Add state_node as child of action_node
                action_node.children.add(state_node)
                # Add action_node as parent of state_node
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node_1 = set(node_a1.action.effect_add) & set(node_a2.action.effect_rem)
        node_2 = set(node_a1.action.effect_rem) & set(node_a2.action.effect_add)

        return bool(node_1 | node_2)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        node1_add = set(node_a1.action.effect_add) & set(node_a2.action.precond_neg)
        node1_rem = set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)
        node2_add = set(node_a2.action.effect_add) & set(node_a1.action.precond_neg)
        node2_rem = set(node_a2.action.effect_rem) & set(node_a1.action.precond_pos)

        return bool(node1_add | node1_rem | node2_add | node2_rem)
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for level, state_nodes in enumerate(self.s_levels):
                if PgNode_s(goal, True) in state_nodes:
                    level_sum += level
                    break
